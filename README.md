...
Transcription uses whisper.cpp for local, offline speech-to-text. Transcriptions run automatically after recordings complete (fire-and-forget) and can also be triggered on-demand from the UI. A serial queue processes one transcription at a time to avoid CPU overload. Voice Activity Detection (VAD) is enabled by default via Silero, skipping silent sections for faster transcription and fewer hallucinations. CPU thread count is configurable via `WHISPER_THREADS` for resource-constrained servers. A transcription queue dialog in the header shows active and pending jobs with live progress.
...
| File | What to change |
|---|---|
| `/etc/default/auris` | `ALSA_DEVICE`, `LISTEN_DEVICE`, `PLAYBACK_DEVICE`, `CAPTURE_STREAM`, `CAPTURE_RECORD`, `RECORDINGS_DIR`, `ICECAST_SOURCE_PASSWORD`, `AUTH_USERNAME`, `AUTH_PASSWORD_HASH`, `COMPRESSOR_*`, `WHISPER_LANGUAGE`, `WHISPER_THREADS`, `WHISPER_VAD`, `WHISPER_VAD_MODEL` |
| `.env.local` | `RECORDINGS_DIR`, `DATABASE_PATH`, `NEXT_PUBLIC_STREAM_URL`, `AUTH_SECRET`, `AUTH_TRUST_HOST`, `NEXTAUTH_URL`, `WHISPER_BIN`, `WHISPER_MODEL` (see `.env.example`) |

...
Remote audio console for monitoring, recording, and two-way communication. Streams live audio via WebSocket, records directly to disk (server and client), and provides push-to-talk intercom. Controlled through a Next.js web UI.
...
Next.js (port 3000)
  ├─ /ws/monitor  → ffmpeg ALSA → PCM → WebSocket broadcast (live monitoring)
  ├─ /ws/talkback → WebSocket → ffmpeg → ALSA output (push-to-talk)
  └─ Recording    → ffmpeg ALSA → MP3 file (direct, no intermediary)
                          ↑
                    Audio Input (USB/ALSA)
```

Audio is handled directly by ffmpeg processes (no systemd services or Icecast):
- **Monitor**: WebSocket streams raw PCM (s16le, 48kHz, mono) from ALSA to browser. Auto-starts/stops based on connected clients (reference counting).
- **Recording**: Spawns ffmpeg to write ALSA capture directly to MP3 file. Optional compressor filter applied inline.
- **Talkback**: WebSocket receives PCM from browser, pipes through ffmpeg to ALSA output.
...
- **Audio:** ffmpeg (ALSA capture, libmp3lame encoding, PCM streaming)
- **System:** ALSA mixer (amixer), PM2 (production), Nginx (reverse proxy)
- **Browser audio:** WebAudio API (AudioContext, BufferSource scheduling, AnalyserNode for level metering)
...
| `src/hooks/use-listening.ts` | Live audio monitoring (wraps use-monitor-stream) |
| `src/hooks/use-monitor-stream.ts` | WebSocket PCM monitor: connects to /ws/monitor, plays via Web Audio API with jitter buffer |
...
| `server.ts` | Custom HTTP/WebSocket server (monitor + talkback WS upgrade with auth, Next.js handler) |
| `src/lib/monitor-stream.ts` | Server-side ffmpeg ALSA → PCM pipe, WebSocket broadcast (globalThis singleton, ref counting) |
| `src/lib/direct-record.ts` | Direct ALSA → MP3 recording via ffmpeg (globalThis singleton, optional compressor) |
...
| `src/app/api/audio/compressor/route.ts` | GET/POST — compressor config (applies on next recording start) |
| `src/app/api/` | All API routes (status, record, audio, recordings, auth, vox, talkback) |
| `system/` | nginx config, sudoers |
...
- Audio: recordings use MP3 128kbps, 44.1kHz, mono; monitor stream uses raw PCM s16le, 48kHz, mono
- Config file: `/etc/default/auris` — `ALSA_DEVICE`, `LISTEN_DEVICE`, `PLAYBACK_DEVICE`, `RECORDINGS_DIR`, `AUTH_USERNAME`, `AUTH_PASSWORD_HASH`, `COMPRESSOR_ENABLED`, `COMPRESSOR_THRESHOLD`, `COMPRESSOR_RATIO`, `COMPRESSOR_MAKEUP`, `COMPRESSOR_ATTACK`, `COMPRESSOR_RELEASE`, `WHISPER_THREADS`, `WHISPER_VAD`, `WHISPER_VAD_MODEL`, `WHISPER_LANGUAGE`, `WHISPER_TRANSLATE`
- Server playback, talkback, monitor stream, and direct recording all use `globalThis` singletons to survive HMR in dev mode and share state with API routes
- Monitor stream (`monitor-stream.ts`) auto-starts ffmpeg when first WebSocket client connects, auto-stops when last client disconnects (reference counting)
- Direct recording (`direct-record.ts`) spawns ffmpeg to write ALSA capture → MP3 file; handles DB registration, waveform generation, and transcription enqueue on stop
...
- WebSocket endpoints (`/ws/talkback`, `/ws/monitor`) in `server.ts` validate the `authjs.session-token` JWT cookie when auth is enabled
...
- Audio compressor (dynamic range compression) is applied via ffmpeg `acompressor` filter in direct recording. Config persisted in `/etc/default/auris`. Changes take effect on next recording start.
- Compressor UI lives in the Monitor card settings popover (`card-monitor.tsx`). Config is lazy-loaded on popover open, changes debounce 500ms before API call.
...
- If you started monitoring (Listen) during testing, stop it before ending the session. The monitor stream auto-stops when no clients are connected, but closing the browser tab handles this.
